...
public class TaskAnalyzer {
	...
	/**
	 * This method extracts a list of tasks from a sentence given the NLP
	 * sentence map.
	 * 
	 * @param pSentenceMap
	 *            sentence map
	...
	private Map<String, List<IndexedWord>> getPrepositions(
			SemanticGraph pCollapsedDependenciesAnnotation,
			Collection<GrammaticalRelation> pPrepositionRelations,
			IndexedWord pVerb, List<IndexedWord> pAccusatives) {
		...
		for (GrammaticalRelation lPrepositionRelation : pPrepositionRelations) {
			...
			for (IndexedWord lAccusative : pAccusatives) {
				lPreps.addAll(pCollapsedDependenciesAnnotation
						.getChildrenWithReln(lAccusative, lPrepositionRelation));
			}
			lAllPreps.put(lPreposition, new ArrayList<IndexedWord>());
			for (IndexedWord lPrep : lPreps) {

				// filter out empty and generic accusatives, and accusatives
				// that aren't nouns or VBGs
				if (!lPrep.word().trim().equals("")
						&& !Configuration.getInstance().getGenericAccusatives()
								.contains(lPrep.word())
						&& (lPrep.tag().startsWith("NN") || lPrep.tag()
								.startsWith("VBG"))) {
	...
	private List<Task> getTasks(SemanticGraph pCollapsedDependenciesAnnotation,
			List<SemanticGraphEdge> pRelations,
			Collection<GrammaticalRelation> pPrepositionRelations) {
		...
		return getTasksFromDependencySources(pCollapsedDependenciesAnnotation,
				pPrepositionRelations, lRelationSources);
	}

	// get verb and object part of tasks
	private List<Task> getTasksFromDependencySources(
			SemanticGraph pCollapsedDependenciesAnnotation,
			Collection<GrammaticalRelation> pPrepositionRelations,
			List<IndexedWord> pRelationSources) {
		...
		for (IndexedWord lVerb : pRelationSources) {
			...
			if (!lVerb.tag().startsWith("VB")
					|| !lVerb.word().toLowerCase().matches("[a-z ]+")) {
				continue;
			}
			...
			if (Configuration.getInstance().getProgrammingActions().size() == 1) {
				for (IndexedWord lConjVerb : pCollapsedDependenciesAnnotation
						.getChildrenWithReln(lVerb,
				...
				for (IndexedWord lConjVerb : pCollapsedDependenciesAnnotation
					...
					if (lConjVerb.tag().startsWith("VB")
							...
							&& Configuration.getInstance()
									...
									.contains(lConjVerb.lemma().toLowerCase())
							&& !pRelationSources.contains(lConjVerb)) {
						lVerbs.add(lConjVerb);
					}
				}
			...
			List<IndexedWord> lAccusatives = pCollapsedDependenciesAnnotation
					.getChildrenWithReln(lVerb,
							GrammaticalRelation.valueOf("dobj"));
			lAccusatives.addAll(pCollapsedDependenciesAnnotation
					.getChildrenWithReln(lVerb,
							GrammaticalRelation.valueOf("nsubjpass")));

			lAccusatives.addAll(pCollapsedDependenciesAnnotation
					.getParentsWithReln(lVerb,
							GrammaticalRelation.valueOf("rcmod")));
			lAllAccusatives.addAll(lAccusatives);
			List<IndexedWord> lFinalAccusatives = getFinalAccusatives(
					pCollapsedDependenciesAnnotation, lVerbalPhrase,
					lAllAccusatives);
			lVerbalPhrase.setAccusatives(lFinalAccusatives,
					pCollapsedDependenciesAnnotation);
			Map<String, List<IndexedWord>> lAllPreps = getPrepositions(
					pCollapsedDependenciesAnnotation, pPrepositionRelations,
					lVerb, lFinalAccusatives);
			lVerbalPhrase.setPrepositions(lAllPreps,
					pCollapsedDependenciesAnnotation);
	...
	private List<IndexedWord> getFinalAccusatives(
			SemanticGraph pCollapsedDependenciesAnnotation,
		...
		for (IndexedWord lAccusative : pAllAccusatives) {
			...
			pVerbalPhrase.setAccusatives(lAccusativeList,
					pCollapsedDependenciesAnnotation);
			if (!Configuration.getInstance().getGenericAccusatives()
				...
				for (IndexedWord lConjAcc : pCollapsedDependenciesAnnotation
						.getChildrenWithReln(lAccusative,
								GrammaticalRelation.valueOf("conj_and"))) {
					List<IndexedWord> lConjAccList = new ArrayList<IndexedWord>();
					lConjAccList.add(lConjAcc);
					pVerbalPhrase.setAccusatives(lConjAccList,
							pCollapsedDependenciesAnnotation);
					if (!Configuration.getInstance().getGenericAccusatives()
							.contains(pVerbalPhrase.getAccusativeString())
