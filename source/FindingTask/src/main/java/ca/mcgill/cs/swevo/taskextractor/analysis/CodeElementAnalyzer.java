...
public class CodeElementAnalyzer
{
	...
	{
		if (StringUtils.stringEndsWithAny(pCodeElementCandidate, Configuration.getInstance().getExceptionalEndings())
				|| StringUtils.stringStartsWithAny(pCodeElementCandidate, Configuration.getInstance()
						.getExceptionalStarts()))
		{
			return true;
		}
		if (StringUtils.stringContainsAny(pCodeElementCandidate, Configuration.getInstance().getExceptionalContains())
				|| StringUtils.stringMatchesAny(pCodeElementCandidate, Configuration.getInstance()
						.getExceptionalMatches()))
		{
			return true;
		}
		if (StringUtils.stringEqualsAny(pCodeElementCandidate, Configuration.getInstance().getExceptionalEquals()))
		{
			return true;
		}
		return false;
